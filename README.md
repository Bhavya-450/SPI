...
![OUTPUT - SPI MASTER](doc/img4.png)
...
The SPI Slave module is designed to communicate with an SPI Master using the standard SPI signals: CS, SCLK, MOSI, and MISO. The slave does not generate the serial clock or chip-select signal; both are controlled by the master. The slave becomes active only when the master selects it by driving the active-low chip-select signal, CS, to 0.

The module receives serial data from the master through the MOSI line and sends serial data back through the MISO line. It supports full-duplex communication, which means transmission and reception occur at the same time.

The main inputs are:

- clk: Internal system clock used by the slave logic.
  
- rst: Reset signal that clears registers and returns the slave to its idle state.
  
- cs: Active-low chip-select input from the master.
  
- sclk: Serial clock generated by the master.
  
- mosi: Serial data sent by the master.
- 
- tx_data[7:0]: Parallel 8-bit data that the slave must transmit to the master.
- 
The main outputs are:

- miso: Serial data transmitted from the slave to the master.
  
- rx_data[7:0]: The complete 8-bit data received from the master.
  
- done: A one-clock-cycle pulse indicating that one SPI transfer has completed.
- 
The slave detects rising and falling edges of SCLK using delayed versions of sclk and cs. When CS changes from high to low, the slave recognizes that it has been selected. It loads tx_data into the transmit shift register and places its most significant bit, tx_data[7], onto the MISO line before the first SPI clock edge.

During the transfer, the slave samples MOSI on every rising edge of SCLK. Each received bit is shifted into the receive shift register. Since the design is MSB-first, after eight rising clock edges the receive register contains the complete byte transmitted by the master.
On every falling edge of SCLK, the slave shifts its transmit register and places the next data bit on MISO. This ensures that the next MISO bit is stable before the master samples it on the following rising clock edge. This timing corresponds to SPI Mode 0, where CPOL = 0 and CPHA = 0.

When the master drives CS high after eight clock pulses, the slave considers the transfer complete. The received byte is copied from the internal receive shift register to rx_data, and done is asserted for one system-clock cycle. When not selected, MISO remains high, keeping the slave output in its idle state.

For example, if the master sends 8'b01011001 through MOSI and the slave is loaded with 8'b11001010 in tx_data, the slave receives 01011001 in rx_data while simultaneously sending 11001010 to the master through MISO.
In conclusion, the SPI Slave module provides a simple 8-bit, MSB-first, full-duplex SPI communication interface. It receives clock and chip-select control from the master, shifts data in through MOSI, shifts data out through MISO, and generates a completion signal after every successful transfer.
